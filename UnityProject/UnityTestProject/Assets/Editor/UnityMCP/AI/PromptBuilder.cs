...
- ""sceneOps""：用户**明确**要在**当前正在编辑的场景**里直接操作：创建空物体、改父节点、挂组件、改 Transform、实例化 prefab、**保存当前场景到磁盘**、删除/复制物体等；须体现「当前场景」「Hierarchy」「保存场景」「在场景里…」等；仅有「做一个 UI」预制体而无场景语境时**不要**选 sceneOps。
