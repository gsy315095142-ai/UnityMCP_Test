...
**目标**: 实现 AI 驱动的预制体生成，与代码生成并列为两大核心能力
...
- [ ] **打成 Unity Package (UPM)**
  - [ ] 将 `Assets/Editor/UnityMCP` 抽离为独立包结构
  - [ ] 创建 `package.json`（包名如 `com.yourname.unity-mcp`）
  - [ ] 支持通过 Git URL 安装：`Add package from git URL`
  - [ ] 支持通过本地路径安装：`Add package from disk`
  - [ ] 可选：发布到 OpenUPM 供他人使用
  - **目的**: 便于在其他 Unity 项目中复用此插件
...
**Phase 1**: ✅ 已完成（MVP 可运行）
**完成度**: Phase 1 100%
**下一步**: 提交代码后，根据需求决定 Phase 2 或 UPM 打包
