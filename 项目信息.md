...
### 项目目录
项目本地文件夹地址：D:\AI_Program\UnityMCP_Test
...
> **能力演进（2026-03）**：在「生成代码 / 预制体」之上，规划 **Unity 编辑器内操控扩展**（场景层级、实例化预制体、挂组件等），以 **插件内 Direct AI + JSON 操作列表** 实现，**不依赖外接 MCP 客户端**。详细架构与 **阶段 A 实施清单** 见《项目开发计划与进度.md》章节 **「Unity 编辑器操控能力扩展」**。
>
> **当前进度**：**A.2–A.4** 已贯通 — 契约与解析执行、场景 Prompt、`AIQuickCommand` 模式 **场景操控** + **AI判断 → sceneOps**，以及预览 JSON / 执行 / 失败步序提示。**A.5** 仍为可选配置与文档收尾。
...
- **Ollama（本地模型）** - Phase 1 测试首选 ✅
  - 无需 API Key，局域网即可使用
  - 默认测试配置：URL `http://192.168.0.34:11434`，模型 `qwen3.5:35b`
  - API 兼容 OpenAI 格式，便于后续扩展
...
- **Ollama（本地模型）** - Phase 1 测试首选
  - 无需 API Key
  - 默认测试：URL `http://192.168.0.34:11434`，模型 `qwen3.5:35b`
  - 支持自定义 Endpoint
- **其他本地模型** - LM Studio, LocalAI 等
...
{
    // 基础配置
    public AIProvider provider;        // AI 服务商（Ollama/Claude/OpenAI/Azure）
    public string apiKey;              // API Key（Ollama 可留空）
    public string modelName;           // 模型选择（如 qwen3.5:35b）

    // 高级配置
    public string customEndpoint;      // 自定义 API 端点（Ollama 示例: http://192.168.0.34:11434）
    public float temperature = 0.7f;   // 创造性 (0-1)
    public int maxTokens = 4000;       // 最大生成长度
    public float topP = 1.0f;         // 采样参数

    // 安全配置
    public bool encryptApiKey = true;  // 加密存储 API Key（Ollama 无需）
...
|--------|--------|
| 项目路径 | D:\AI_Program\UnityMCP_Test |
| Phase 1 AI 测试 | Ollama，URL: http://192.168.0.34:11434，模型: qwen3.5:35b |
| 开发语言 | 注释和 UI 文本统一使用中文 |
| 生成脚本路径 | Assets/Scripts/Generated/（沿用文档建议） |
| 生成预制体路径 | Assets/Prefabs/Generated/（沿用文档建议） |

### Unity 项目创建说明

**需要用户创建 Unity 项目**。创建完成后告知，将检查已安装的包（XR、Input System、TextMeshPro 等）。

**目录要求**：
- ⚠️ **项目路径不能包含空格**（Unity-MCP 官方要求）
  - ✅ 推荐：`D:\AI_Program\UnityMCP_Test\UnityProject`
  - ❌ 避免：`D:\AI Program\...`
- 建议将 Unity 项目放在工作区内或工作区附近，便于统一管理
- 插件代码将放在 Unity 项目的 `Assets/Editor/UnityMCP/` 下
